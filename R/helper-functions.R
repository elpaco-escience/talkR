...
inspect_language <- function(lang=NULL,saveplot=F,allsources=F) {

  dp <- d |> filter(language == lang)
  nturns <- dp |> drop_na(FTO) |> ungroup() |> summarize(n=n()) |> as.integer()

  pA <- dp |>
    ggplot(aes(FTO)) +
    theme_tufte() +
    ggtitle(paste0(nturns,' transitions')) +
  ...
  pC <- dp |>
    ggplot(aes(duration)) +
    theme_tufte() +
    ggtitle('Turn duration') +
    geom_density(na.rm=T,linewidth=1)

  turntypes <- n_distinct(dp$utterance_stripped)
